...
def parse_arguments():
    ...
    parser.add_argument('--name', type=str, default='my_single_scale_normal2', help='training run name')
    ...
    parser.add_argument('--outdir', type=str, default='../../data/dsacmodels2', help='output folder (trained models)')
    parser.add_argument('--logdir', type=str, default='../../data/dsaclogs2', help='training log folder')
    ...
    #parser.add_argument('--gpu_idx', type=str, default='0,1,2,3', help='set < 0 to use CPU')
...
def train_dsacpnet(opt):
    ...
    dsac = DSAC(opt.hypotheses, opt.inlierthreshold, opt.inlierbeta, opt.inlieralpha, opt.normal_loss,opt.seed)
    #dsacpnet.to(device)
    if opt.seed < 0:
    ...
    for epoch in range(opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            print('[%s %d: %d/%d] %s loss: %f Top Loss:%f' % (opt.name, epoch, train_batchind, train_num_batch-1, green('train'), exp_loss.item(),top_loss))
            ...
            while test_fraction_done <= train_fraction_done and test_batchind+1 < test_num_batch:
                ...
                # forward pass
                with torch.no_grad():
                    exp_loss, top_loss = dsac(points,target)
               ...
               #exp_loss, top_loss = dsac(
                ...
                # print info and update log file
                print('[%s %d: %d/%d] %s loss: %f Top Loss:%f' % (opt.name, epoch, train_batchind, train_num_batch-1, blue('test'), exp_loss.item(),top_loss))
                test_writer.add_scalar('loss', exp_loss.item(), (epoch + test_fraction_done) * train_num_batch * opt.batchSize)
