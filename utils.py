...
def pts_to_plane(q, out=None):

    #batchsize = q.size(0)

    #if out is None:
     #   out = q.new_empty(batchsize,4)
    out = []
    out.append((q[:,2,2]-q[:,2,0])*(q[:,1,1]-q[:,1,0]) -(q[:,1,2]-q[:,1,0])*(q[:,2,1]-q[:,2,0]))
    out.append((q[:,0,2]-q[:,0,0])*(q[:,2,1]-q[:,2,0]) -(q[:,2,2]-q[:,2,0])*(q[:,0,1]-q[:,0,0]))
    out.append((q[:,1,2]-q[:,1,0])*(q[:,0,1]-q[:,0,0]) -(q[:,0,2]-q[:,0,0])*(q[:,1,1]-q[:,1,0]))
    out.append(-out[0]*q[:,0,0]-out[1]*q[:,1,0]-out[2]*q[:,2,0])
    out=torch.stack(out)
